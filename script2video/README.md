...
全部为 **SIL Open Font License**，免费可商用。字文件随工具打包（[Fontsource](https://fontsource.org/) 的 woff2），**不请求 Google Fonts**。离线导出也是同一套。自定义舞台 CSS 也不要写系统字体名。
...
顶栏「导出」：默认不烧录字幕条、不另存字幕文件，需要时在窗里勾选。
