...
function summarizeTool(name: string, result: string): { ok: boolean; summary: string } {
  try {
    const data = JSON.parse(result) as { error?: string; ok?: boolean; count?: number; mode?: string; name?: string };
    if (data.error) return { ok: false, summary: `${name}：${data.error}` };
    if (name === "apply_storyboard") return { ok: true, summary: `已${data.mode === "append" ? "追加" : "写入"} ${data.count ?? 0} 场` };
    if (name === "update_scene") return { ok: true, summary: "已更新场景" };
    if (name === "get_project" || name === "get_scene" || name === "list_catalog") return { ok: true, summary: `已读取 ${name}` };
